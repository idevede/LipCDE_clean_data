...
def get_processed_data_cf(raw_sim_data,
                       b_predict_actions,
                       b_use_actions_only,
                       b_use_predicted_confounders,
                       b_use_oracle_confounders,
    ...
    for i in range(sequence_lengths.shape[0]):
        ...
        if not b_predict_actions:
            for k in range(horizon):
                #include the censoring point too, but ignore future shifts that don't exist
                active_entries[i, int(sequence_length/2):sequence_length-k, k] = 1
                print('unknown  active_entries')
        else:
            active_entries[i, int(sequence_length/2):sequence_length, :] = 1
