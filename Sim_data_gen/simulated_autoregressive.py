...
class AutoregressiveSimulation:
    ...
    def generate_data_single_patient(self, timesteps):
        ...
        history_cf_0 = dict()
        history_cf_0['covariates'] = [x_0]
        history_cf_0['confounders'] = [z_0]
        history_cf_0['treatments'] = [a_0]

        history_cf_1 = dict()
        history_cf_1['covariates'] = [x_0]
        history_cf_1['confounders'] = [z_0]
        history_cf_1['treatments'] = [a_0]


        for t in range(int(timesteps/2)):
            ...
            history_cf_0['covariates'].append(x_t)
            history_cf_0['confounders'].append(z_t)
            history_cf_0['treatments'].append(a_t)

            history_cf_1['covariates'].append(x_t)
            history_cf_1['confounders'].append(z_t)
            history_cf_1['treatments'].append(a_t)

        
        for t in range(int(timesteps/2), int(timesteps)):
            x_t = self.generate_covariates_single_timestep(self.p, history)
            z_t = self.generate_confounders_single_timestep(self.p, history)
            ...
            a_t_0 = np.zeros_like(a_t)
            x_t_0 = self.generate_covariates_single_timestep(self.p, history_cf_0)
            z_t_0 = self.generate_confounders_single_timestep(self.p, history_cf_0)
            history_cf_0['covariates'].append(x_t_0)
            history_cf_0['confounders'].append(z_t_0)
            history_cf_0['treatments'].append(a_t_0)

            
            a_t_1 = np.ones_like(a_t)
            x_t_1 = self.generate_covariates_single_timestep(self.p, history_cf_1)
            z_t_1 = self.generate_confounders_single_timestep(self.p, history_cf_1)
            history_cf_1['covariates'].append(x_t_1)
            history_cf_1['confounders'].append(z_t_1)
            history_cf_1['treatments'].append(a_t_1)

        return np.array(history['covariates']), np.array(history['confounders']), np.array(history['treatments']), \
                np.array(history_cf_0['covariates']), np.array(history_cf_0['confounders']), np.array(history_cf_0['treatments']), \
                    np.array(history_cf_1['covariates']), np.array(history_cf_1['confounders']), np.array(history_cf_1['treatments']) 
    ...
    def generate_dataset(self, num_patients, max_timesteps, binary_outcome=False):
            ...
            dataset_cf_0 = dict()
            dataset_cf_0['previous_covariates'] = []
            dataset_cf_0['previous_treatments'] = []
            dataset_cf_0['covariates'] = []
            dataset_cf_0['confounders'] = []
            dataset_cf_0['treatments'] = []
            dataset_cf_0['sequence_length'] = []
            dataset_cf_0['outcomes'] = []

            dataset_cf_1 = dict()
            dataset_cf_1['previous_covariates'] = []
            dataset_cf_1['previous_treatments'] = []
            dataset_cf_1['covariates'] = []
            dataset_cf_1['confounders'] = []
            dataset_cf_1['treatments'] = []
            dataset_cf_1['sequence_length'] = []
            dataset_cf_1['outcomes'] = []

            for patient in range(num_patients):
                timesteps = np.random.randint(int(max_timesteps)-10, int(max_timesteps), 1)[0]
                covariates_history, confounders_history, treatments_history, \
                covariates_history_cf_0, confounders_history_cf_0, treatments_history_cf_0, \
                covariates_history_cf_1, confounders_history_cf_1, treatments_history_cf_1    = self.generate_data_single_patient(
                    timesteps + 1)
                ...
                previous_covariates = np.vstack((np.array(covariates_history_cf_0[1:timesteps - 1]),
                                                 np.zeros(shape=(max_timesteps-timesteps, self.num_covariates))))
                previous_treatments = np.vstack((np.array(np.array(treatments_history_cf_0[1:timesteps - 1])),
                                                 np.zeros(shape=(max_timesteps-timesteps, self.num_treatments))))

                covariates = np.vstack((np.array(covariates_history_cf_0[1:timesteps]),
                                        np.zeros(shape=(max_timesteps - timesteps, self.num_covariates))))

                confounders = np.vstack((np.array(confounders_history_cf_0[1:timesteps]),
                                        np.zeros(shape=(max_timesteps - timesteps, self.num_confounders))))

                treatments = np.vstack((np.array(treatments_history_cf_0[1:timesteps]),
                                        np.zeros(shape=(max_timesteps-timesteps, self.num_treatments))))


                outcomes = self.gamma_y * np.mean(np.array(confounders_history_cf_0[2:timesteps + 1]), axis=-1) + \
                           (1-self.gamma_y) * np.mean(np.array(confounders_history_cf_0[2:timesteps + 1]), axis=-1)
                ...
                dataset_cf_0['previous_covariates'].append(np.array(previous_covariates))
                dataset_cf_0['previous_treatments'].append(np.array(previous_treatments))
                dataset_cf_0['covariates'].append(np.array(covariates))
                dataset_cf_0['confounders'].append(np.array(confounders))
                dataset_cf_0['treatments'].append(np.array(treatments))
                dataset_cf_0['sequence_length'].append(np.array(timesteps))
                dataset_cf_0['outcomes'].append(np.array(outcomes))


                previous_covariates = np.vstack((np.array(covariates_history_cf_1[1:timesteps - 1]),
                                                 np.zeros(shape=(max_timesteps-timesteps, self.num_covariates))))
                previous_treatments = np.vstack((np.array(np.array(treatments_history_cf_1[1:timesteps - 1])),
                                                 np.zeros(shape=(max_timesteps-timesteps, self.num_treatments))))

                covariates = np.vstack((np.array(covariates_history_cf_1[1:timesteps]),
                                        np.zeros(shape=(max_timesteps - timesteps, self.num_covariates))))

                confounders = np.vstack((np.array(confounders_history_cf_1[1:timesteps]),
                                        np.zeros(shape=(max_timesteps - timesteps, self.num_confounders))))

                treatments = np.vstack((np.array(treatments_history_cf_1[1:timesteps]),
                                        np.zeros(shape=(max_timesteps-timesteps, self.num_treatments))))


                outcomes = self.gamma_y * np.mean(np.array(confounders_history_cf_1[2:timesteps + 1]), axis=-1) + \
                           (1-self.gamma_y) * np.mean(np.array(confounders_history_cf_1[2:timesteps + 1]), axis=-1)
                ...
                dataset_cf_1['previous_covariates'].append(np.array(previous_covariates))
                dataset_cf_1['previous_treatments'].append(np.array(previous_treatments))
                dataset_cf_1['covariates'].append(np.array(covariates))
                dataset_cf_1['confounders'].append(np.array(confounders))
                dataset_cf_1['treatments'].append(np.array(treatments))
                dataset_cf_1['sequence_length'].append(np.array(timesteps))
                dataset_cf_1['outcomes'].append(np.array(outcomes))
            ...
            for key in dataset_cf_0.keys():
                dataset_cf_0[key] = np.array(dataset_cf_0[key])
            for key in dataset_cf_0.keys():
                dataset_cf_1[key] = np.array(dataset_cf_1[key])
