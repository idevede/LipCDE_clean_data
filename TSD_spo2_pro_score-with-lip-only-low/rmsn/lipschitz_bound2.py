...
class LipschitzBound:
  ...
  def __init__(self, kernel_shape, padding, sample=50, backend='torch',
    ...
    cout, cin, ksize, ksize2 = kernel_shape
   ...
   # create samples
    if self.backend == 'numpy':
      ...
      H0 = p_index.repeat(ksize2).reshape(ksize2, ksize).T.reshape(-1)
      H1 = p_index.repeat(ksize2)
      if self.cuda:
        H0 = H0.cuda()
        H1 = H1.cuda()
  ...
  def _compute_from_torch_v1(self, kernel):
    ...
    poly1 = torch.einsum('ijk,ijk->jk', poly_real, poly_real)
    poly2 = torch.einsum('ijk,ijk->jk', poly_imag, poly_imag)
    poly = poly1 + poly2
    sv_max = torch.sqrt(poly.max(axis=-1)[0].sum())
    return sv_max, poly
