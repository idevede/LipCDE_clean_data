...
def get_processed_data(raw_sim_data,
    ...
    # Binary application
    treatments = raw_sim_data['treatments'] # [4005, 30, 1]
    covariates = raw_sim_data['covariates'] #[4005, 30, 3]
    predicted_confounders = raw_sim_data['predicted_confounders'] #【4005, 30, 5]
    dataset_outputs = raw_sim_data['outcomes'] #[4005, 30, 1]
    sequence_lengths = raw_sim_data['sequence_length'] #[4005]
...
def get_processed_sc_data(raw_sim_data,
                       b_predict_actions,
                       b_use_actions_only,
                       b_use_predicted_confounders,
                       b_use_oracle_confounders,
    ...
    # Parcelling INPUTS
    if b_use_predicted_confounders:    
        inputs = np.concatenate([covariates[:, 1:, ], predicted_confounders[:, 1:, ], treatments[:, :-1, ]], axis=2) #【4005，29，5】
        #inputs = np.concatenate([covariates[:, 1:, ], treatments[:, :-1, ]], axis=2) #【4005，29，5】
        #inputs =covariates[:, 1:, 8:] #np.concatenate([covariates[:, 1:, ]], axis=2) #【4005，29，5】
    else:
        inputs = np.concatenate([covariates[:, 1:, ], treatments[:, :-1, ]], axis=2) #【4005，29，5】
    ...
    if b_pro_Score:
        propensity_score = raw_sim_data['propensity_score']
    if b_pro_Score:
        return {'outputs': outputs,  # already scaled
                'scaled_inputs': inputs,
                'scaled_outputs': outputs,
