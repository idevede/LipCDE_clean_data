...
class AutoregressiveSimulation:
    ...
    def generate_dataset(self, num_patients, max_timesteps, binary_outcome=False):
        ...
        # #dataset = load_results('../TSD_pytorch_rnn_baseline_to_improve_MIMIC/mimic_data/dataset_mimic.txt')
        ...
        #dataset_mimic = load_results('/home/defucao/workspace/TSD_pytorch_rnn_baseline_to_improve_MIMIC/mimic_data/1228/dataset_mimic_30_10_meanbp_raw_less.txt')
        #dataset_mimic = load_results('/meladyfs/newyork/defucao/defu/data/processed_data/fin_data/dataset_mimic_30_10_meanbp_raw.txt')
        dataset_mimic = load_results('/meladyfs/newyork/defucao/defu/data/processed_data/fin_data/dataset_mimic_30_10_spo2_raw.txt')
        covariates = dataset_mimic['covariates']
        dataset_mimic['previous_covariates'] = dataset_mimic['previous_covariates'][:,:-2,:-1]
        dataset_mimic['previous_treatments'] = dataset_mimic['previous_treatments'][:,:-2,:]
        dataset_mimic['covariates'] = covariates[:,:-1,:-1]
        dataset_mimic['confounders'] = covariates[:,:-1,-1:]
        dataset_mimic['predicted_confounders'] = covariates[:,:-1,-1:]
        dataset_mimic['treatments'] = dataset_mimic['treatments'][:,:-1,:]
        #dataset_mimic['sequence_length'].append(np.array(timesteps))
        dataset_mimic['outcomes'] = dataset_mimic['outcomes'][:, :-1, :]

        dataset = dataset_mimic
        dataset = self.normalize_dataset(dataset_mimic)
