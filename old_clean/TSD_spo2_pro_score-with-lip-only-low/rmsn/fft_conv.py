...
from torch.fft import irfftn, rfftn, fftshift, ifftshift
import math
...
def fft_conv(
    ...
    M = c/2
    N = l/2
    d0 =5 # GLPF filter, d0=5, 15, 30 , https://www.cnblogs.com/pingwen/p/12442257.html， 现在是低通
    for i in range(b):
        for j in range(c):
            for k in range(l):
                d = math.sqrt(math.pow((j - M), 2) + math.pow((k - N), 2))
                #h = 1 - 1*math.exp(-1/2*(math.pow(d,2)/(d0*d0)))
                h = 1*math.exp(-1/2*(math.pow(d,2)/(d0*d0)))
                signal_fr_shift[i][j,k]= h * signal_fr_shift[i][j,k]
    signal_fr_shift = ifftshift(signal_fr_shift)
    kernel_fr = rfftn(padded_kernel, dim=tuple(range(2, signal.ndim)))
...
class _FFTConv(nn.Module):
    ...
    def __init__(
        ...
        padding: Union[int, Iterable[int]] = 1,
        stride: Union[int, Iterable[int]] = 1,
